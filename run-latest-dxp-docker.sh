...
# Choose which type of tag to default to: any, updates (u), or long-term support (lts)
RELEASE_TYPE_DEFAULT=any
read_config "Release type (any|u|lts)" RELEASE_TYPE "$RELEASE_TYPE_DEFAULT"

API_BASE='https://hub.docker.com/v2/repositories/liferay/dxp/tags?page_size=2048&ordering=name'

# Build the query and regex based on the chosen release type
case "${RELEASE_TYPE:l}" in
  ...
  u)
    ...
    PATTERN='^[0-9]{4}\.q[0-9]+\.[0-9]+-u[0-9]+$'
    ;;
  *)
    QUERY_URL="$API_BASE"
    PATTERN='^[0-9]{4}\.q[0-9]+\.[0-9]+(?:-(?:u[0-9]+|lts))?$'
    ;;
esac

# Try to find the latest tag for the current year first
LIFERAY_TAG_DEFAULT=$(curl -s "$QUERY_URL" \
| jq -r --arg year "$(date +%Y)" --arg pat "$PATTERN" '
    .results[].name
    | select(startswith($year))
    | select(test($pat))
  ' | sort -V | tail -n1)

# If none found for the current year, fall back to the latest across all years
...
# If still empty, warn so the prompt has no misleading default
if [[ -z "$LIFERAY_TAG_DEFAULT" ]]; then
